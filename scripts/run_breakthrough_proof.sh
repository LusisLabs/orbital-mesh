...
run_live_chaos() {
  ...
  docker compose "${PROJECT_ARGS[@]}" run --rm --no-deps \
    ...
    -e "MESH_STACK_CHAOS_OPERATOR_ID=${MESH_STACK_CHAOS_OPERATOR_ID:-mesh-compose-chaos}" \
    -e "MESH_STACK_CHAOS_OPERATOR_ROLES=${MESH_STACK_CHAOS_OPERATOR_ROLES:-launcher,viewer}" \
    "${CHAOS_SERVICE}"
}
