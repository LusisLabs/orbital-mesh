...
class MeshControlPlaneRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/api/runs":
            ...
            return
        if parsed.path == "/api/mesh-brain/mvp-runs":
            try:
                run = self.server.coordinator.run_mesh_brain_mvp(payload)
            except ValueError as exc:
                self._send_json({"error": str(exc)}, status=HTTPStatus.BAD_REQUEST)
                return
            except Exception as exc:  # pragma: no cover - defensive; avoid empty TCP replies to clients
                _LOG.exception("POST /api/mesh-brain/mvp-runs failed: %s", exc)
                self._send_json(
                    {"error": "mesh brain mvp run failed", "detail": str(exc)},
                    status=HTTPStatus.INTERNAL_SERVER_ERROR,
                )
                return
            self._send_json(run, status=HTTPStatus.CREATED)
        ...
        if parsed.path == "/api/mesh-brain/live-serving-smoke":
            ...
            self._send_json(run, status=HTTPStatus.CREATED)
            return
        if parsed.path == "/api/mesh-brain/live-adapter-runtime-probe":
            try:
                run = self.server.coordinator.run_mesh_brain_live_adapter_runtime_probe(payload)
            except ValueError as exc:
                self._send_json({"error": str(exc)}, status=HTTPStatus.BAD_REQUEST)
                return
            except Exception as exc:  # pragma: no cover - defensive; avoid empty TCP replies to clients
                _LOG.exception("POST /api/mesh-brain/live-adapter-runtime-probe failed: %s", exc)
                self._send_json(
                    {"error": "mesh brain live adapter runtime probe failed", "detail": str(exc)},
                    status=HTTPStatus.INTERNAL_SERVER_ERROR,
                )
                return
            self._send_json(run, status=HTTPStatus.CREATED)
        ...
        if parsed.path == "/api/mesh-brain/backend-matrix":
            ...
            self._send_json(run, status=HTTPStatus.CREATED)
            return
        if parsed.path == "/api/mesh-brain/posttraining-proof":
            try:
                run = self.server.coordinator.run_mesh_brain_posttraining_proof(payload)
            except ValueError as exc:
                self._send_json({"error": str(exc)}, status=HTTPStatus.BAD_REQUEST)
                return
            except Exception as exc:  # pragma: no cover - defensive; avoid empty TCP replies to clients
                _LOG.exception("POST /api/mesh-brain/posttraining-proof failed: %s", exc)
                self._send_json(
                    {"error": "mesh brain posttraining proof failed", "detail": str(exc)},
                    status=HTTPStatus.INTERNAL_SERVER_ERROR,
                )
                return
            self._send_json(run, status=HTTPStatus.CREATED)
            return
        if parsed.path == "/api/mesh-brain/mlx-lm-lora-e2e":
            try:
                run = self.server.coordinator.run_mesh_brain_mlx_lm_lora_e2e(payload)
            except ValueError as exc:
                self._send_json({"error": str(exc)}, status=HTTPStatus.BAD_REQUEST)
                return
            except Exception as exc:  # pragma: no cover - defensive; avoid empty TCP replies to clients
                _LOG.exception("POST /api/mesh-brain/mlx-lm-lora-e2e failed: %s", exc)
                self._send_json(
                    {"error": "mesh brain MLX-LM-LoRA e2e failed", "detail": str(exc)},
                    status=HTTPStatus.INTERNAL_SERVER_ERROR,
                )
                return
            self._send_json(run, status=HTTPStatus.CREATED)
