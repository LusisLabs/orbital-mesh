...
fn runMacos(app: zero_native.App, options: RunOptions, init: std.process.Init) !void {
    ...
    var app_info = options.appInfo();
    const store = prepareStateStore(init.io, init.environ_map, &app_info, &buffers);
    var mac_platform = try zero_native.platform.macos.MacPlatform.initWithOptions(zero_native.geometry.SizeF.init(1360, 860), webEngine(), app_info);
...
fn runLinux(app: zero_native.App, options: RunOptions, init: std.process.Init) !void {
    ...
    var app_info = options.appInfo();
    const store = prepareStateStore(init.io, init.environ_map, &app_info, &buffers);
    var linux_platform = try zero_native.platform.linux.LinuxPlatform.initWithOptions(zero_native.geometry.SizeF.init(1360, 860), webEngine(), app_info);
...
fn runWindows(app: zero_native.App, options: RunOptions, init: std.process.Init) !void {
    ...
    var app_info = options.appInfo();
    const store = prepareStateStore(init.io, init.environ_map, &app_info, &buffers);
    var windows_platform = try zero_native.platform.windows.WindowsPlatform.initWithOptions(zero_native.geometry.SizeF.init(1360, 860), webEngine(), app_info);
