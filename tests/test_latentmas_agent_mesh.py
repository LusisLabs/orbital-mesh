...
class LatentMasAgentMeshTests(unittest.TestCase):
    ...
    def test_latentmas_enabled_prepends_completed_attempt(self) -> None:
        ...
        try:
            ...
            self.assertEqual(task.attempts[0].output["authority"]["state_slice"], "mesh.agent_attempt_thread.v1")
            self.assertEqual(task.attempts[0].output["authority"]["boundary"], "advisory_only")
            self.assertFalse(task.attempts[0].output["authority"]["production_actuation_allowed"])
