...
def verify_agentic_operator_source_provenance(path: str | Path | None) -> dict[str, Any]:
    ...
    recorded_snapshot_sha = str(provenance.get("source_snapshot_sha256") or "") if provenance else ""
    snapshot_sha = _snapshot_sha256(source_root) if source_root_available and source_root else None
    snapshot_matches = bool(provenance and source_root_available and snapshot_sha == recorded_snapshot_sha)
    snapshot_recorded = bool(provenance and not source_root_available and recorded_snapshot_sha)
    ...
    source_commit_unavailable_but_snapshotted = bool(
        ...
        and (snapshot_matches or snapshot_recorded)
    )
    ...
    return {
        ...
        "source_snapshot_recorded": snapshot_recorded,
