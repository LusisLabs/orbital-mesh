...
`scripts/verify_load_concurrency_rehearsal.py --proof <load-concurrency-rehearsal.json> --json` verifies a `mesh.load_concurrency_rehearsal.v1` packet for production-expansion readiness.
...
This closes the proof contract for the capacity lane. It does not substitute for a target staging or production-expansion load run.
...
`scripts/verify_authenticated_ingress_deployment.py --proof "$MESH_AUTHENTICATED_INGRESS_PROOF_PATH" --json` verifies a `mesh.authenticated_ingress_deployment_proof.v1` packet from the deployed proxy. The proof must show non-local environment, operator id, HTTPS ingress URL, TLS termination, identity-provider enforcement, Mesh header stripping, proxy-stamped identity and roles, viewer/launcher/approver/admin group mappings, private upstream boundary, passing app-level rehearsal evidence, audit identity/source evidence, and no raw secret material. Staging readiness now reports `authenticated_ingress_deployment_verified`; production Compose requires `MESH_AUTHENTICATED_INGRESS_PROOF_PATH` so a deployment cannot claim external human-test readiness from app-level role checks alone.
...
- `shared/mesh_runtime/load_concurrency.py`, `shared/mesh_runtime/schemas/load-concurrency-rehearsal.schema.json`, `scripts/verify_load_concurrency_rehearsal.py`, `MESH_LOAD_CONCURRENCY_REHEARSAL_PATH`, and expansion readiness gate `load_concurrency_rehearsal_verified` now make multi-operator load, queue backpressure, tenant quota, target-lock conflict, cancellation, stuck-run recovery, admission latency, and event persistence latency proof machine-checkable. Evidence level: focused proof-schema, CLI, readiness, and docs contract proof; remaining gap: target staging or production-expansion operators still need to capture a real load/concurrency rehearsal packet;
...
- real partner-specific design-partner packet with documented consent and live go/no-go/release-provenance hashes;
