...
- [x] Slice 6 `hardened-arena.api-surface.v1`: bounded profiles/catalog/packet create/packet lookup/intent create routes, operator identity for stored packet and intent creation, and no deployment/secret/apply route.
- [x] Slice 7 `hardened-arena.meshapp-ui.v1`: Meshapp Build Arena wizard, profile/use/compliance selection, component graph, authority boundaries, blockers, proof checklist, packet generation, intent preparation, and visible no-deploy/no-secret/no-kubeconfig posture without deployed-state language.
