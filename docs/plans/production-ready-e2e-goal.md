...
| P14 | blocked | `release-provenance-current-head.v1` | Release assurance | Release provenance and runtime-binding tests pass locally, and the repo-local commit step retired the `clean_git_tree` blocker. A complete strict release packet is still missing for the current checked-out head. Current local kubectl-source-build rehearsal evidence binds image digest `sha256:af36481183033d5dd3aa0e7a9542068ebdbb71e3e87e4c0fb893f3bb34f1eef8`, 6 unique base image digests, migration rehearsal, signed policy lifecycle packet, build command, and a real CycloneDX SBOM with 4827 components for the pre-commit rehearsal window. Current blockers: strict generation still lacks release-supplied `image_digest`, `base_image_digests`, `policy_lifecycle_signed`, `migration_rehearsal`, `sbom_path`, `vulnerability_scan_path`, `ci_attestation`, and `build_command`; the normalized Grype scan still has 14 unaccepted high/critical findings; the exception policy expired on 2026-05-21; and `scripts/verify_release_runtime_binding.py --json` still lacks `MESH_RELEASE_PROVENANCE_PATH`. | `scripts/generate_release_provenance.py --require-complete --json`, `scripts/verify_release_runtime_binding.py --json`, `pnpm run test:focused` |
