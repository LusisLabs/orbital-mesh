...
| P14 | blocked | `release-provenance-current-head.v1` | Release assurance | Release provenance and runtime-binding tests pass locally, but the current dirty `HEAD` is missing a complete strict release packet. Current local kubectl-source-build rehearsal evidence now binds image digest `sha256:af36481183033d5dd3aa0e7a9542068ebdbb71e3e87e4c0fb893f3bb34f1eef8`, 6 unique base image digests, migration rehearsal, signed policy lifecycle packet, build command, and a real CycloneDX SBOM with 4827 components. Current blockers: the normalized Grype scan still has 14 unaccepted high/critical findings, the exception policy expired on 2026-05-21, GitHub Actions CI attestation is missing, and strict generation still requires a clean git tree plus release-supplied inputs. | `scripts/generate_release_provenance.py --require-complete --json`, `scripts/verify_release_runtime_binding.py --json`, `pnpm run test:focused` |
