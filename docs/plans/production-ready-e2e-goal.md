...
| P14 | blocked | `release-provenance-current-head.v1` | Release assurance | Release provenance, runtime-binding, role-bundle, scan-normalization, and verified-VEX contracts pass locally. Current dirty-tree role candidates have zero unaccepted high or critical findings under the existing exception policy plus exact digest-bound VEX: control plane `30 = 28 exceptions + 2 VEX`, authority `3 = 2 + 1`, verifier `3 = 2 + 1`. This is local candidate evidence, not a current-head registry release. Current blockers are the private-repository GitHub artifact-attestation entitlement, exhausted Actions artifact storage, missing verifier public-policy variables, one clean exact-commit three-role build/publication/attestation/bundle run, complete control-plane release provenance, and deployed-runtime binding. | `scripts/generate_release_provenance.py --require-complete --json`, `scripts/verify_repo_patch_service_image_bundle.py`, `scripts/verify_release_runtime_binding.py --json`, `pnpm run test:focused` |
...
The first evidence paragraph below records the historical 2026-05-23 state and is superseded by the dated 2026-07-13 corrections that follow it.
...
2026-07-13 superseding local remediation result: Docker CLI `v29.6.1` and kubectl `v1.36.2` are now built from pinned source with Go `1.26.5`, and the release-assurance generator applies a proof-bound, exact-predicate correction for Syft `1.44.0` issue 5057 while preserving the full raw SBOM. A current-database control-plane scan now has 30 blocking findings, 28 existing-policy acceptances, and two unaccepted findings: `CVE-2026-15308` on Python `3.13.14` and `CVE-2026-7017` on `perl-base` `5.40.1-6`. Authority and verifier retain one unaccepted `CVE-2026-15308` finding each. No new exception was added. The manual-dispatch-only exact-commit GitHub workflow builds, scans, publishes, attests, and verifies all three immutable role images only after every role reaches zero unaccepted findings and explicit GitHub attestation-entitlement and artifact-storage preconditions pass. P14 remains blocked on the four role-level findings, the current private-repository attestation entitlement, exhausted Actions artifact storage, missing verifier public-policy variables, live workflow evidence, and deployed-runtime binding; the dirty-tree local image is measurement evidence only and is not a current-head release artifact.

2026-07-13 verified-VEX implementation correction: all three role images now install the exact SHA-256-checked CPython 3.13 backport for `CVE-2026-15308`. `mesh.release_vulnerability_evidence.v1` retains the full Grype scan and binds reviewed determinations to the exact image digest, raw scan hash, raw match hash, package/version/matcher predicates, and isolated runtime proof. It admits only the patched Python finding as `fixed` and the exact Debian 13 `perl-base` `CVE-2026-7017` source-package match as `not_affected` after proving the vulnerable `HTTP::Tiny` code is absent. This removes neither raw findings nor the existing exception ledger and fails closed on drift. P14 remains blocked until rebuilt same-database scans prove zero unaccepted findings and the separate GitHub entitlement, storage, public-policy variables, live publication/attestation, and deployed-runtime gates close.

2026-07-13 same-database VEX closure: local Syft `1.44.0` and Grype `0.112.0` with database schema `v6.1.7`, built `2026-07-13T07:22:23Z`, produced zero unaccepted blocking findings for all three rebuilt role candidates. The control-plane image `sha256:61aac0dd90f098900bbebfaf3396c4641413551a2457578937f802392b066478` retained `177` findings and `30` high/critical findings, classified as `28` existing-policy acceptances plus two verified-VEX records. Authority `sha256:e89d650a93f6e694bf42a8d527c63c145730baebb68cd4d16cd3a97c3e9c0ea4` retained `12` findings and `3` blockers, classified as `2 + 1`; verifier `sha256:db4cfe1e3a6cb4ca5b67753fcc42edd67a525aee9ea4dafaa6b2d6ac6b5bfda4` retained `11` findings and `3` blockers, also `2 + 1`. The role-bundle validator accepted each raw Grype file, VEX record, normalized scan, image digest, and raw-match fingerprint as one chain. These local image ids were built from the working source tree and are not GHCR manifest digests, exact-commit GitHub attestations, or deployed-runtime proof. P14 therefore remains blocked on platform and live-release evidence, not on a residual local scanner finding.
