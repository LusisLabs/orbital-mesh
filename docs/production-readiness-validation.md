...
| Web lint/contracts | `pnpm run lint` | PASS | Reran on 2026-05-26 through the root pnpm `verify:full` gate after adding legacy web unit and browser E2E to the heavy gate. The run included contracts, focused Python tests, meshapp unit tests, product Playwright, web unit tests, legacy web Playwright, Praxis proof packet verification, web lint, meshapp lint, and handover package verification. |
| Web unit tests | `pnpm --dir web run test` | PASS | Reran on 2026-05-26 through pnpm; `3` files passed, `16` tests passed. The same command is now part of the root `verify:full` gate. |
| Web build | `pnpm --dir web run build` | PASS WITH WARNING | Reran on 2026-05-26 through pnpm. Vite build exited `0`; it still reports `dist/assets/index-BnYtysjL.js` at `718.96 kB`, above the 500 kB warning threshold. |
| Product app build | `pnpm --dir meshapp/frontend run build` | PASS | Reran on 2026-05-26 through pnpm. Next.js production build, TypeScript, static page generation, and export completed successfully. |
| UI Labyrinth/Playwright | `pnpm --dir web run test:e2e` | PASS | Reran on 2026-05-26 through pnpm after replacing the legacy runner's npm invocation and log-tail failure path. Playwright reported `14 passed` across desktop and mobile, and the command is now part of the root `verify:full` gate. |
...
| Pilot readiness/go-no-go | `GET /api/readiness` and `GET /api/pilot/go-no-go` | PASS, HISTORICAL LOCAL E2E OVERLAY; NOT A CLEAN RELEASE CUT | On 2026-05-23 the E2E overlay was started with `MESH_E2E_BUILD_COMMIT=$(git rev-parse HEAD) docker compose -f docker-compose.stack.yml -f docker-compose.e2estack.yml up -d --build mesh` for head `cfd9f3b18d0d0bd87a59056faaa442ae73994573`. The overlay supplies Postgres, k3s targets, authenticated ingress proof, backup/restore proof, migration proof, on-call drill proof, feature-flag/incident/audit provider proofs, Mesh Brain artifact registry/upload proof, and a local OpenAI-compatible Mesh Brain serving stub. The control plane observed Mesh Brain model-kernel run `run_20260523T052801_58d01fb1`, live canary smoke run `run_20260523T053440_0bfaa6dd`, exactly one CROPS canary lane for `tenant_a`, rollback drill run `run_20260523T052801_12a88f6f`, denied-action run `run_20260523T050025_ed9817d1`, and approved smoke run `run_20260523T043031_f55fbea8`. `docker exec -w /workspace/orbital-mesh orbital-mesh-stack python3 scripts/verify_pilot_clearance.py --base-url http://127.0.0.1:8787 --timeout-seconds 45 --expected-head cfd9f3b18d0d0bd87a59056faaa442ae73994573 --json` returned `status=pass`, `mode=clearance`, `/api/health status=ok`, `/api/readiness status=ready`, `/api/pilot/go-no-go status=go`, empty `missing_evidence`, release provenance packet SHA `3027c613c9088247a1ae092df35a5606891896c4ccb52515000e86388f29ae20`, runtime image digest `sha256:eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee`, and runtime commit matching that historical head. The captured local packet `.mesh-runtime-state/e2e/pilot-go-no-go.json` also verified with signed operator signoff `.mesh-runtime-state/e2e/pilot-signoff.json` using `scripts/verify_pilot_signoff.py --signoff .mesh-runtime-state/e2e/pilot-signoff.json --go-no-go .mesh-runtime-state/e2e/pilot-go-no-go.json --signing-key mesh-e2e-pilot-signoff-key --json`, which returned `status=pass`. Boundary: the overlay release provenance uses `compose-e2e` CI attestation and the explicit E2E digest, so this does not clear the current branch head, P14 clean release provenance, or same-target production proof slices. |
| Release provenance / CI artifacts | CI and handoff artifacts plus `mesh.release_provenance.v1` completion and runtime-binding checks | PARTIAL, CURRENT-HEAD PROVENANCE INCOMPLETE | After the repo-local commit step and local image rebuild on 2026-05-23, strict provenance generation returned `status=incomplete` with `git.dirty=false` and only `vulnerability_scan_path` plus `ci_attestation` missing. The ignored local rehearsal `.mesh-runtime-state/release-provenance-p14-current-rehearsal.json` must be regenerated after each commit before it is cited as current-head evidence; the latest regeneration bound image digest, base image digests, migration rehearsal, policy signing key, build command, and real Syft/Grype artifacts. The SBOM is valid and digest-matched with 4827 components, but the vulnerability scan is invalid for release signing because it has 14 unaccepted high/critical findings in Python `3.13.13`, glibc, and ncurses packages. Docker CLI `29.5.2` removed the duplicate Docker binary Go stdlib findings; source-built kubectl `v1.36.1` is now built with `go1.26.3`, which removes the kubectl Go stdlib findings, and the configured exception policy expired on 2026-05-21. The generated local CI attestation records `provider=local` and is intentionally not accepted as GitHub Actions proof. `scripts/verify_release_runtime_binding.py --json` still fails with missing `release_provenance_path`. The E2E overlay release packet is local compose evidence only and does not replace a clean signed CI release packet. The 2026-05-10 handoff runtime mounted a complete release packet for then-current commit `583eb3e2335cb416e1360d9f0b2cbd3420e04275`, image digest `sha256:c54e7e28b9d94cb8ecdb07b60048a9bf3cf5ce853362c37089955ff8d9303d3a`, and packet SHA `db7a5d5b6cddfcf9994605672f984e10cb72ebcef8f9e3cbec86ad33855d2904`; that is historical for the current head. The older CI run `25525840560` and image digest `sha256:2c088dd6ae51e97f9560fbc9e65ff564d0ec173afdb33121b41219fa8684da2f` remain historical evidence for commit `803b13e51f984a27f4bf42d0014ebb8d50cdd26a` only. |
...
| HelixDB memory projection | `PYTHONPATH=. python3 scripts/verify_helix_memory_projection.py --json --require-enabled --replay-pending` | PASS (OPTIONAL) | Tested on 2026-05-11 with `MESH_MEMORY_GRAPH_BACKEND=helix` enabled. The script verifies that `helix_memory.py` projection accepts `upsert_observation`, `upsert_claim`, `upsert_relationship`, `upsert_supersession`, `record_retrieval`, and `upsert_memory_packet` operations; the `--replay-pending` flag replays any failed outbox events before probe execution. This is an optional feature gate, not required for pilot clearance. |
...
Run `scripts/verify_pilot_clearance.py --base-url http://127.0.0.1:8787 --timeout-seconds 30 --expected-head "$(git rev-parse HEAD)" --json` as the final current-head local audit before any release-bound pilot-clearance claim. It fails unless `/api/health` is healthy, `/api/readiness` is `pilot` and ready, `/api/pilot/go-no-go` is `go` with no missing evidence, the embedded release provenance record is complete, the runtime health metadata matches the release packet commit and image digest, and `/api/health.commit` matches the supplied `--expected-head`.

For the full frontend/backend e2e release claim, `pnpm run lint` is necessary but
not sufficient. Run `pnpm run verify:e2e-claim` with explicit current-head
artifact inputs before using production-ready language. The verifier fails
closed until a downloaded release artifact bundle, release runtime binding,
pilot clearance endpoint, and P24 aggregate autonomy proof all pass for the same
...
- The earlier `.mesh-runtime-state/release-provenance.json` packet for commit
  `803b13e51f984a27f4bf42d0014ebb8d50cdd26a` is historical. The 2026-05-10
  live handoff runtime mounted a complete release packet at
  `/release-provenance.json` for then-current commit
  ...
  with packet SHA
  `db7a5d5b6cddfcf9994605672f984e10cb72ebcef8f9e3cbec86ad33855d2904`. That
  proves runtime binding only for that historical handoff commit; readiness and
  go/no-go remained blocked. The 2026-05-11 localhost runtime reports
  `commit=unknown` and `image_digest=null`, so it is not current-head release
  proof for any later branch head.
...
- Pilot readiness and go/no-go must be treated as runtime-bound, not
  ...
  still returned `status=fail` because readiness and go/no-go evidence were
  blocked. The 2026-05-11 localhost runtime reports `commit=unknown`; it does
  not clear the current `origin/main` head. Historical green rows do not clear
  the current runtime.
