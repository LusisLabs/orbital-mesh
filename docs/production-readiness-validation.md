...
| Release provenance / CI artifacts | CI run `25468774490` plus `mesh.release_provenance.v1` draft inspection | BLOCKED, EVIDENCE PRESERVED | Live CI reran on 2026-05-07 UTC at branch commit `c1cf1a2`. It uploaded `ci-attestation` artifact `6844624566`, `release-assurance-artifacts` `6844624754`, and `release-provenance-draft` `6844624887`; local copies are under `/tmp/orbital-mesh-ci-25468774490/`. The attestation is real GitHub Actions evidence with `python-test` and `web` passed and `docker-build` failed. The draft provenance packet is still incomplete: the SBOM is digest-matched, but the vulnerability scan has `18` high/critical blockers and the CI attestation is intentionally invalid for pilot signing because `docker-build` did not pass. |
