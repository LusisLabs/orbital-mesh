...
| Trigger web source provenance | `PYTHONPATH=. python3 scripts/verify_trigger_web_source_provenance.py --json` | PASS | Added for the Trigger-derived operator UI surface. Verifies the Trigger web source paths, authority gates, forbidden imports, and required source files match `config/trigger-web-source-provenance.json`. This is provenance contract proof, not live Trigger runtime verification. |
