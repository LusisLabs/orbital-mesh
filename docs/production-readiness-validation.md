...
| Pilot readiness/go-no-go | `GET /api/readiness` and `GET /api/pilot/go-no-go` | BLOCKED, NO CURRENT-HEAD RUNTIME BINDING | Reran `curl -sS --max-time 2 http://127.0.0.1:8787/api/health` on 2026-05-11 from `origin/main` commit `d327b03825d95649728816cad449f7f2dae472fc`; the live localhost runtime returned `status=ok`, `environment=production`, `commit=unknown`, and `image_digest=null`, so it is not current-head pilot proof. The 2026-05-10 `scripts/verify_pilot_clearance.py --base-url http://127.0.0.1:8787 --timeout-seconds 30 --json` result is preserved as prior runtime-bound evidence for handoff commit `583eb3e2335cb416e1360d9f0b2cbd3420e04275` and image digest `sha256:c54e7e28b9d94cb8ecdb07b60048a9bf3cf5ce853362c37089955ff8d9303d3a`; that verifier still returned `status=fail` because readiness and go/no-go were blocked. The 2026-05-08 green result remains historical for commit `803b13e51f984a27f4bf42d0014ebb8d50cdd26a` only. |
| Release provenance / CI artifacts | CI and handoff artifacts plus `mesh.release_provenance.v1` completion and runtime-binding checks | PARTIAL, CURRENT-HEAD PROVENANCE INCOMPLETE | Current-head provenance generation on 2026-05-11 from clean `origin/main` commit `d327b03825d95649728816cad449f7f2dae472fc` returns `status=incomplete`; missing release inputs are `image_digest`, `base_image_digests`, `policy_lifecycle_signed`, `migration_rehearsal`, `sbom_path`, `vulnerability_scan_path`, `ci_attestation`, and `build_command`. The 2026-05-10 handoff runtime mounted a complete release packet for then-current commit `583eb3e2335cb416e1360d9f0b2cbd3420e04275`, image digest `sha256:c54e7e28b9d94cb8ecdb07b60048a9bf3cf5ce853362c37089955ff8d9303d3a`, and packet SHA `db7a5d5b6cddfcf9994605672f984e10cb72ebcef8f9e3cbec86ad33855d2904`; that is historical for the current `origin/main` head. The older CI run `25525840560` and image digest `sha256:2c088dd6ae51e97f9560fbc9e65ff564d0ec173afdb33121b41219fa8684da2f` remain historical evidence for commit `803b13e51f984a27f4bf42d0014ebb8d50cdd26a` only. |
...
Run `scripts/verify_pilot_clearance.py --base-url http://127.0.0.1:8787 --timeout-seconds 30 --json` as the final local audit before any release-bound pilot-clearance claim. It fails unless `/api/health` is healthy, `/api/readiness` is `pilot` and ready, `/api/pilot/go-no-go` is `go` with no missing evidence, the embedded release provenance record is complete, and the runtime health metadata matches the release packet commit and image digest. For current-head clearance, also require the runtime commit to match the checked-out head; PR #27 adds that explicit verifier flag but is blocked on GitHub Actions billing until CI can run.
...
- The earlier `.mesh-runtime-state/release-provenance.json` packet for commit
  ...
  proof for `d327b03825d95649728816cad449f7f2dae472fc`.
