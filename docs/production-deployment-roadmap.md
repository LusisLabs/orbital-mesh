...
| Supply-chain provenance | private staging | `scripts/generate_release_provenance.py` emits a release packet and `--require-complete` fails without CI artifacts, migration rehearsal, SBOM, vulnerability scan, CI attestation, image digest, and clean signed CI packet generation. CI now collects built image/base-image metadata, runs pinned Syft and Grype scanners through `scripts/generate_release_image_assurance.py`, feeds the digest metadata into `mesh.ci_attestation.v1`, and uploads an incomplete release-provenance draft for review. |
