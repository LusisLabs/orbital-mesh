...
| Supply-chain provenance | private staging | `scripts/generate_release_provenance.py` emits a release packet and `--require-complete` fails without CI artifacts, migration rehearsal, SBOM, vulnerability scan, CI attestation, image digest, and clean signed CI packet generation. CI now collects built image/base-image metadata, feeds it into `mesh.ci_attestation.v1`, and uploads an incomplete release-provenance draft for review. |
...
| Release provenance packet | production pilot | Local generator and completeness gate exist; CI must supply image digests, base-image digests, SBOM, vulnerability scan, CI attestation, clean tree, build command, and builder identity before a signed pilot packet is valid. |
...
| Deployment compatibility matrix | private staging | Initial contract is documented in `docs/deployment-compatibility.md`: Docker Compose and Kubernetes are validated paths; OCI/containerd/runc compatibility is contract-level; Podman, K3s, OpenShift, Rancher-managed Kubernetes, managed Kubernetes, Cloud Run, Azure Container Apps, Fly.io, Railway, and Render remain recipes until target-specific smoke evidence exists; ECS/Fargate is the next validated non-Kubernetes target; Swarm, Mesos/Marathon, and Windows Containers are not planned. |
...
| Role-stamped approvals and handoffs | production pilot | Implemented; approvals record operator id, roles, source, and event id. Operator handoffs now emit `mesh.operator_handoff.v1`, persist as run artifacts, and appear in run export packages and archives. |
