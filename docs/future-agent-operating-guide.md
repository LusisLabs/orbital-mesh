...
- Operator UI: `meshapp/` for the production pilot-serving app and zero-native shell, especially `meshapp/frontend/src/App.tsx`, `meshapp/frontend/src/api.ts`, `meshapp/frontend/src/types.ts`, `meshapp/frontend/src/lib/`, and `meshapp/src/`. `apps/mesh-webapp` is the planned Trigger-derived Remix dashboard surface after adaptation. `web/` remains the Vite reference surface during migration until parity is proven.
