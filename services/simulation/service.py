...
class SimulationService:
    ...
    def _builtins(self) -> list[SimulationScenario]:
        ...
        node_pressure = _otel_metric_signal("sig_otel_node_pressure_001", "k8s.node.memory.pressure", 0.4, 0.86, "semantic-search")
        storage_pressure = _otel_metric_signal("sig_otel_storage_pressure_001", "k8s.volume.available", 0.42, 0.08, "semantic-search")
        dns_error = _otel_metric_signal("sig_otel_dns_error_001", "dns.lookup.error_rate", 0.01, 0.12, "api-gateway")
        ingress_tls = _otel_metric_signal("sig_otel_ingress_tls_001", "ingress.tls.handshake_failure_rate", 0.01, 0.16, "api-gateway")
        ...
        return [
            ...
            SimulationScenario(
                ...
                title="Kubernetes node pressure scale-out",
                signal_payload=node_pressure,
                expected_decision_type="scale_deployment",
                expected_outcome="successful",
                fault_type="node_pressure",
                scenario_family="capacity",
                sandbox={"kube_context": "mesh-compose", "namespace": "search", "deployment": "semantic-search"},
                tags=["kubernetes", "node_pressure", "scale"],
                standards_refs=["kubernetes-skew-policy", "otel-semconv-1.40.0"],
            ),
            SimulationScenario(
                scenario_id="k8s_storage_pressure_escalate",
                title="Kubernetes storage pressure escalation",
                signal_payload=storage_pressure,
                expected_decision_type="escalate",
                expected_outcome="successful",
                fault_type="storage_pressure",
                scenario_family="storage",
                sandbox={"kube_context": "mesh-compose", "namespace": "search", "deployment": "semantic-search"},
                tags=["kubernetes", "storage", "human_review"],
                standards_refs=["kubernetes-skew-policy", "nist-ai-rmf"],
            ),
            SimulationScenario(
                scenario_id="dns_error_escalate",
                title="DNS error-rate escalation",
                signal_payload=dns_error,
                expected_decision_type="escalate",
                expected_outcome="successful",
                fault_type="dns_error_rate",
                scenario_family="networking",
                sandbox={"kube_context": "mesh-compose", "namespace": "sandbox-payments", "deployment": "api-gateway"},
                tags=["dns", "networking", "human_review"],
                standards_refs=["otel-semconv-1.40.0", "nist-ai-rmf"],
            ),
            SimulationScenario(
                scenario_id="ingress_tls_escalate",
                title="Ingress TLS handshake failure escalation",
                signal_payload=ingress_tls,
                expected_decision_type="escalate",
                expected_outcome="successful",
                fault_type="ingress_tls_failure",
                scenario_family="networking",
                sandbox={"kube_context": "mesh-compose", "namespace": "sandbox-payments", "deployment": "api-gateway"},
                tags=["ingress", "tls", "human_review"],
                standards_refs=["otel-semconv-1.40.0", "nist-ai-rmf"],
            ),
            SimulationScenario(
                scenario_id="db_latency_escalate",
                title="Database client latency escalation",
                signal_payload=db_latency,
                expected_decision_type="escalate",
                expected_outcome="successful",
            ...
            SimulationScenario(
                scenario_id="queue_poison_messages_escalate",
                title="Queue poison-message escalation",
                signal_payload=queue_poison,
                expected_decision_type="escalate",
                expected_outcome="successful",
                fault_type="queue_poison_messages",
                scenario_family="queue",
                sandbox={"kube_context": "mesh-compose", "namespace": "search", "deployment": "semantic-search"},
                tags=["queue", "poison_message", "human_review"],
                standards_refs=["otel-semconv-1.40.0", "nist-ai-rmf"],
            ),
            SimulationScenario(
                scenario_id="argocd_drift_escalate",
                title="ArgoCD drift escalation",
                signal_payload=argocd_drift,
                expected_decision_type="escalate",
                expected_outcome="successful",
                fault_type="argocd_drift",
                scenario_family="gitops",
                sandbox={"kube_context": "mesh-compose", "namespace": "search", "deployment": "semantic-search"},
            ...
            SimulationScenario(
                ...
                title="Feature-flag rollback conflict escalation",
                signal_payload=flag_conflict,
                expected_decision_type="escalate",
                expected_outcome="successful",
                fault_type="feature_flag_rollback_conflict",
                scenario_family="feature_flag",
                sandbox={"kube_context": "mesh-compose", "namespace": "search", "deployment": "api-gateway"},
                tags=["feature_flag", "rollback_conflict", "human_review"],
                standards_refs=["nist-ai-rmf", "nist-ai-600-1"],
            ),
            SimulationScenario(
