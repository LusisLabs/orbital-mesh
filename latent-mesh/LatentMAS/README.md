...
## Rust Core and Latent Briefing

This fork adds a Rust orchestration core for hardened experiment plumbing and Latent Briefing compaction planning. It provides typed agent roles, local dataset loading, prompt rendering, answer evaluation, and a MAD-threshold shared-mask planner for task-guided KV cache compaction.
...
The existing Python Hugging Face/vLLM backend remains available through the Rust CLI:

```bash
cargo run -- --python-backend --method latent-mas --model-name Qwen/Qwen3-14B --task gsm8k -- --max_samples 1
```

See `docs/latent-briefing-rust.md` for the public Rust API boundary and Latent Briefing planner details.

TextMAS now trims prior-agent context by tokenizer IDs before adding it to later prompts:

```bash
python run.py --method text_mas --model_name Qwen/Qwen3-14B --task gsm8k --text_mas_context_tokens 2048
```

Use `--text_mas_context_tokens -1` to disable token trimming.
